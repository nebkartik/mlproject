...
class DataTransformation:
    ...
    def get_data_transformer_object(self):
        try:
            ...
            logging.info('Numerical & Categorical Transformation Completed')
    ...
    def initiate_data_transformation(self,train_path,test_path):
        try:
            ...
            logging.info('Feature Dropped')
            # logging.info("Data After dropping: ", input_feature_train_df.head())

            preprocessor_obj = self.get_data_transformer_object()
            input_feature_train_arr = preprocessor_obj.fit_transform(input_feature_train_df)
            logging.info("Preprocessor shape",input_feature_train_arr.shape)
            input_feature_test_arr = preprocessor_obj.fit_transform(input_feature_test_df)
            logging.info("Preprocessor Applied!")
            train_arr = np.c_[input_feature_train_arr,np.array(train_df[target_feature_name])]
            test_arr = np.c_[input_feature_test_arr,np.array(test_df[target_feature_name])]
            save_obj(
